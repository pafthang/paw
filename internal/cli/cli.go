...
func runServe(ctx context.Context, cmd *cobra.Command, args []string) error {
	...
	if host, _ := cmd.Flags().GetString("host"); host != "" { settings.WebHost = host }
	if port, _ := cmd.Flags().GetInt("port"); port > 0 { settings.WebPort = port }
	return server.New(settings).Run(ctx)
}

func runChat(ctx context.Context, out io.Writer, cmd *cobra.Command, args []string) error {
	settings, err := config.Load(); if err != nil { return err }
	model, _ := cmd.Flags().GetString("model"); if model == "" { model = llm.DefaultModel(settings) }
...
func runTool(ctx context.Context, out io.Writer, name string, input string, sessionID uint) error {
	...
	if !json.Valid(raw) { return fmt.Errorf("invalid input JSON") }
	database, err := db.Open(); if err != nil { return err }
	runner := agent.NewRunner(database, tools.DefaultRegistry())
	resp, err := runner.Run(ctx, agent.RunRequest{SessionID: sessionID, ToolCalls: []agent.ToolCall{{Name: name, Input: raw}}}); if err != nil { return err }
